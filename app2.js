var year = new Date().getFullYear(); // return current year 2021 
var day = new Date().getDay();    // return current day mumber i.e for mon= 2,sun=0 etc
var month = new Date().getMonth(); // return current  month number i.e 0,1,2,
var date = new Date().getDate();   // return current date
...
//checks for leap year to set no of days in a month
...
// function to check number of date in month
...
//Set Top bar for calendar
  ...
  const setFirstDay=(setYear, setMonth ) => {
    var monthdate = new Date(setYear, setMonth , 1).getDay(); // get date in numirical number sun = 0
